...
def _get_vdb_op(
    ...
    if CONFIG.vector_store.name == "milvus":
    ...
    elif CONFIG.vector_store.name == "kdbai":
        from nvidia_rag.utils.vdb.kdbai.kdbai_vdb import KdbaiVDB

        # Map search_type to KDB.AI metric
        # dense -> L2 (Euclidean), hybrid not fully supported in KDB.AI
        metric = "L2"
        if CONFIG.vector_store.search_type == "hybrid":
            # KDB.AI doesn't have built-in BM25 hybrid, use cosine for semantic
            metric = "CS"

        # Read custom metadata CSV if provided
        if csv_file_path is not None:
            meta_dataframe = pandas_file_reader(csv_file_path)
        else:
            meta_dataframe = None
        ...
        vdb_kwargs = {
            "collection_name": collection_name,
            "kdbai_endpoint": vdb_endpoint or CONFIG.vector_store.url,
            "api_key": os.getenv("KDBAI_API_KEY"),
            "database_name": os.getenv("KDBAI_DATABASE", "default"),
            "embedding_model": embedding_model,
            "index_type": os.getenv("KDBAI_INDEX_TYPE", "hnsw"),
            "metric": metric,
            # GPU configurations (for KDB.AI cuVS image)
