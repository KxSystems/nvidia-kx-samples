...
## Release 2.4.0 (2026-06-03)

This release adds KDB-X (bare kdb+) as a second vector database backend with full GPU CAGRA
support via NVIDIA cuVS, validates self-hosted LLM deployment on Blackwell g7e (RTX PRO 6000)
in EKS, adds NeMo Guardrails Helm chart support, and updates embed and rerank model names to
the new llama-nemotron branding.

### Highlights

This release contains the following key changes:

- Added KDB-X as a new vector database backend. KDB-X connects to a customer-managed bare kdb+
  endpoint over IPC, giving teams that already operate kdb+ infrastructure a zero-migration path
  into the RAG Blueprint. The KDB-X process loads the required `.rag.*` definitions from
  `kdbx-init.q` at q startup (automatic in the chart's test pod; the adapter fails fast if they
  are missing — it does not push them on connect). For details, refer to
  [Change Vector Database to KDB-X](change-vectordb-kdbx.md).
- Added GPU-accelerated CAGRA vector search for KDB-X collections via NVIDIA cuVS. Enable with
  `KDBX_USE_CUVS=1` on the kdb+ pod. Note that both `APP_VECTORSTORE_ENABLEGPUINDEX=True` and
  `APP_VECTORSTORE_ENABLEGPUSEARCH=True` must also be set on the `rag-server` and `ingestor`
  pods — `kdbx.useCuvs=true` in Helm alone is not sufficient. See
  [Known Issues](#all-known-issues).
- Added cuVS safety guards for KDB-X CAGRA: a kill-switch environment variable
  `KDBX_CAGRA_SKIP_PERSISTED_READ=1` (or `kdbx.cagra.skipPersistedRead=true` in Helm) lets
  operators recover from a corrupted `.cagra` blob by rebuilding the index from stored vectors.
  A GPU stamp (`_cagrastamp`) and a readiness canary (`.rag.cuvs.canary`) verify cuVS health
  before the pod accepts traffic.
- Added Helm chart support for NeMo Guardrails. Previous releases supported NeMo Guardrails
  only via Docker Compose. The chart is working end-to-end: safe queries receive answers and
  unsafe queries are blocked.
- Validated self-hosted LLM deployment (`llama-3.3-nemotron-super-49b-v1.5`, FP8, TP1) on
  g7e instances (RTX PRO 6000 Blackwell, 96 GB) in EKS. Reference overlay files:
  `deploy/EKS/g7e-llm-nodegroup.yaml` and `deploy/EKS/rag-values-llm-selfhost-g7e.yaml`.
- Added KDB-X per-collection metric persistence. The distance metric (L2 or cosine similarity)
  is stamped at collection creation time and survives pod restarts. Configurable via the
  `KDBX_METRIC` environment variable (default: L2).
- Added `deploy/EKS/rag-values-nim-cache.yaml` overlay to provision PVCs for pre-cached NIM
  model weights in airgapped EKS deployments.
- Renamed embed and rerank model references to the new llama-nemotron branding:
  - `nvidia/llama-3.2-nv-embedqa-1b-v2` is now `nvidia/llama-nemotron-embed-1b-v2`
  - `nvidia/llama-3.2-nv-rerankqa-1b-v2` is now `nvidia/llama-nemotron-rerank-1b-v2`
  - `llama-3.2-nemoretriever-1b-vlm-embed-v1` is now `llama-nemotron-embed-vl-1b-v2`
- Renamed the KDB-X listen-port environment variable from `KDBX_PORT` to `KDBX_LISTEN_PORT`
  (breaking change). A Kubernetes Service named `kdbx` automatically injects
  `KDBX_PORT=tcp://...` into pod environments, which collided with the previous variable name.
  Update any custom configurations that reference `KDBX_PORT`.
...
- Fixed Helm-based deployment for NeMo Guardrails (previously unsupported).
...
- **KDB-X CAGRA double-flag requirement**: Setting `kdbx.useCuvs=true` in Helm alone does not
  activate GPU CAGRA index creation for KDB-X collections. Both the `rag-server` and `ingestor`
  pods must also have `APP_VECTORSTORE_ENABLEGPUINDEX=True` and `APP_VECTORSTORE_ENABLEGPUSEARCH=True`
  set via `envVars`. The `deploy/EKS/rag-values-kdbx-cuvs.yaml` overlay sets all required values.
