...
AIVA_NIM -> KDBX: Log interactions

== Job Scheduling and Execution ==
ADMIN -> API: Create new job
API -> KDBX: Store job metadata
API -> REDIS: Queue job task
API -> ADMIN: Return job ID

REDIS -> CELERY: Dequeue job task
CELERY -> KDBX: Update job status (RUNNING)
CELERY -> KDBX: Pull evaluation data
...
== Reading Results ==
CELERY -> KDBX: Store evaluation results
CELERY -> KDBX: Update job status (COMPLETED)
...
- KDB-X and Redis are managed services

### Development Focus
The Flywheel Service represents the primary development effort for this blueprint:
- All other components (NeMo microservices, AIVA) are existing services
- The Flywheel Service must be built from scratch
- Key components to implement:
  - FastAPI REST interface
  - Celery task management
  - Redis queue integration
  - KDB-X data platform integration (metadata, logging, vector search)
...
### KDB-X Logging Schema
All interactions are logged to KDB-X using the following Pydantic model:

```python
class LogEntry(BaseModel):
    """Schema for logging interactions to KDB-X"""
...
- Data persistence:
  ...
  - KDB-X for application state, logging, and vector search
- Workflow components:
  - Dataset creation from KDB-X
  - NIM deployment and management
  - Evaluation pipeline (F1-score + financial backtest)
...
- KDB-X for data platform (metadata, logging, vector search, market data)
