...
HELM_CHART_VERSION="25.12.0"  # Empty string means use latest version
...
install_dependency() {
  ...
  case "$tool" in
    ...
    huggingface-cli)
      ...
      # Determine if we should use --user flag: not for root or virtualenv
      local use_user_flag=false
      if ! is_root && [[ -z "${VIRTUAL_ENV:-}" ]] && ! python3 -c "import sys; exit(0 if sys.prefix == sys.base_prefix else 1)" 2>/dev/null; then
        use_user_flag=true
      fi
      
      # Install with or without --user flag
...
Options:
  --helm-chart-version VER    Override the default helm chart version (default: latest)
                          Example: --helm-chart-version 25.12.0
