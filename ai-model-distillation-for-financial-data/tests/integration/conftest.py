...
# ---------------------------------------------------------------------------
# Shared helper — insert log records into KDB-X flywheel_logs
# ---------------------------------------------------------------------------
...
def insert_log_records(
    ...
    """Insert records into the ``flywheel_logs`` table and return doc IDs.

    Each record must contain ``request`` and ``response`` keys.  The helper
    fills in ``doc_id``, ``workload_id``, ``client_id``, and ``timestamp``
    if they are missing.
    """
    doc_ids: list[str] = []
    for record in records:
        doc = dict(record)
        doc_id = doc.pop("_doc_id", None) or str(ObjectId())
        doc["doc_id"] = doc_id
        doc.setdefault("workload_id", workload_id)
        doc.setdefault("client_id", client_id)
        doc.setdefault("timestamp", "2023-01-01T00:00:00Z")
...
def db():
    """Fixture to provide a KDB-X database connection for each test."""
...
# Backward-compatible alias so existing tests using ``mongo_db`` still work.
@pytest.fixture(scope="session")
def mongo_db(db):
    """Alias for ``db`` — kept for backward compatibility."""
    yield db
...
def create_flywheel_run_generic(test_workload_id: str, client_id: str):
    ...
    init_db()
    kdbx_db = get_db()
    result = kdbx_db.flywheel_runs.insert_one(flywheel_run.to_mongo())
    return str(result.inserted_id), kdbx_db
...
def mock_embedding_nim() -> Generator[MagicMock, None, None]:
    ...
    with patch("src.lib.nemo.embedding.Embedding") as mock_embedding_class:
