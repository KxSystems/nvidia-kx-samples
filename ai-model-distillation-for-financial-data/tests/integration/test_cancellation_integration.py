...
def verify_subsequent_tasks_skipped(
    ...
    Args:
        ...
        mongo_db: KDB-X database connection for verification
...
def load_cancellation_test_data():
    ...
    tc_records = [
        ...
        for i, rec in enumerate(aiva_records)
    ]
    insert_log_records(db, tc_records, "test-toolcalling-cancellation", "test-client-toolcalling-cancellation")
    ...
    insert_log_records(db, generic_records, "test-generic-cancellation", "test-client-generic-cancellation")

    yield

    try:
        db.flywheel_logs.delete_many({"client_id": "test-client-toolcalling-cancellation", "workload_id": "test-toolcalling-cancellation"})
        db.flywheel_logs.delete_many({"client_id": "test-client-generic-cancellation", "workload_id": "test-generic-cancellation"})
...
def mock_embedding_services():
    ...
    with (
        ...
        # Setup KDB-X embeddings mocking
...
class TestCancellationWorkflowIntegration:
    ...
    def test_full_workflow_cancellation_during_customization(self, cancel_environment, mongo_db):
        ...
        custom_result = ensure_task_result(start_customization(base_eval_result))
