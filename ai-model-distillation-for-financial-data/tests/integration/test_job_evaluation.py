...
def load_test_data_fixture(workload_id: str, client_id: str) -> Generator:
    ...
    from src.scripts.load_test_data import load_data_to_kdbx

    logger = setup_logging("data_flywheel.test_fixture")

    # Load test data for this specific test
    load_data_to_kdbx(workload_id, client_id, file_path="aiva-test.jsonl")
    ...
    for retry_count in range(max_retries):
        rows = db.flywheel_logs.find({"workload_id": workload_id, "client_id": client_id})
        if len(rows) > 0:
            logger.info(f"Data verification successful: found {len(rows)} records")
            break
        if retry_count < max_retries - 1:
            logger.warning(
                f"Data not yet available (attempt {retry_count + 1}/{max_retries}), waiting..."
            )
            time.sleep(0.5)
    else:
        raise RuntimeError(
    ...
    try:
        db.flywheel_logs.delete_many({"workload_id": workload_id, "client_id": client_id})
        logger.info(
            f"Cleaned up test records for workload_id={workload_id}, client_id={client_id}"
        )
...
def cleanup_test_data():
    ...
    Recreates the KDB-X tables with ``drop_existing=True`` to ensure a
    clean slate.  Each test uses unique workload/client IDs so
    cross-test interference is unlikely, but a full reset is safer for
    integration tests.
    """
    from kdbx.schema import create_all_tables

    # Cleanup before test — drop and recreate all tables
