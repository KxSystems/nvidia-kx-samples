...
class TestJobServiceCreateDatasets:
    ...
    def test_create_dataset_success(
        ...
        assert len(create_result.datasets) == 2  # BASE + TRAIN (ICL removed)
        ...
        assert len(job_json["datasets"]) == 2
        for dataset in job_json["datasets"]:
            assert dataset["name"] in ["dataset_1", "dataset_2"]
            assert dataset["num_records"] == 50
            assert dataset["nmp_uri"] in ["uri_1", "uri_2"]
