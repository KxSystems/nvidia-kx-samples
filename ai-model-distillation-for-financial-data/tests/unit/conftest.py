...
def _es_hits_to_dataframe(es_mock):
    """Convert ES-style search return_value into a pandas DataFrame.

    This bridges old test code that sets ``mock_es_client.search.return_value``
    with the new RecordExporter that reads from KDB-X via ``pykx_connection``.
    """
    search_data = es_mock.search.return_value
    if not search_data:
        return pd.DataFrame()
...
def _make_pykx_bridge(es_mock):
    """Build a pykx_connection replacement that delegates to es_mock settings.

    * ``es_mock.search.return_value`` → DataFrame returned by ``q(...).pd()``
    * ``es_mock.search.side_effect``  → exception raised by ``q(...)``
    """

    class _QResult:
        def pd(self):
            return _es_hits_to_dataframe(es_mock)

    class _QConn:
        def __call__(self, *a, **kw):
            se = es_mock.search.side_effect
            if se is not None:
...
def mock_es_client():
    ...
    Tests configure data via ``mock_es_client.search.return_value`` (ES-style
    hits dict).  The fixture transparently bridges that to a ``pykx_connection``
    mock so ``RecordExporter.get_records()`` returns equivalent records.
    """
...
def tweak_settings(monkeypatch):
    ...
    # --- Disable enrichment in unit tests (avoids real KDB-X calls) --------
