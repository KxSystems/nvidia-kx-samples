...
centralising KDB-X specifics in one place.  It also enables unit tests that
...
class TaskDBManager:
    """Lightweight façade over KDB-X tables used by Celery tasks."""
    ...
    def update_enrichment_stats(
        self, flywheel_run_id: str | ObjectId, stats: dict
    ) -> None:
        """Store enrichment metadata on the flywheel run document."""
        self._flywheel_runs.update_one(
            {"_id": ObjectId(flywheel_run_id)},
            {"$set": {"enrichment_stats": stats}},
        )
    ...
    def delete_job_records(self, job_id: ObjectId) -> None:
        """Delete all KDB-X records related to a job.
