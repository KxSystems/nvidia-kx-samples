...
# NOTE: SimilarityConfig, EmbeddingConfig (NIM subclass), and ICLConfig were
# removed as part of the ICL deprecation cleanup.  The standalone
# EmbeddingConfig below (line ~280) is for *embedding NIM deployments*, not ICL.
...
class EnrichmentConfig(BaseModel):
    """Config for market data enrichment of training records."""

    enabled: bool = False
    sym_field: str = "sym"
    timestamp_field: str = "timestamp"
    sym_extraction: str = "field"  # "field" or "regex"
    sym_regex: str = r"\b([A-Z]{1,5})\b"
    default_sym: str = "SPY"


class BacktestConfig(BaseModel):
    """Config for backtest assessment."""
...
class Settings(BaseSettings):
    ...
    def from_yaml(cls, config_path: str | Path) -> "Settings":
        ...
        with open(config_path) as f:
            ...
            enrichment_config = (
                EnrichmentConfig(**config_data["enrichment_config"])
                if "enrichment_config" in config_data
                else EnrichmentConfig()
            )
            backtest_config = (
                BacktestConfig(**config_data["backtest_config"])
            ...
            llm_judge_config = (
                LLMJudgeConfig.from_json(config_data["llm_judge_config"])
