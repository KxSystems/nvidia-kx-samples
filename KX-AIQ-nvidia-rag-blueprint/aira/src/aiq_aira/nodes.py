...
async def generate_query(state: AIRAState, config: RunnableConfig, writer: StreamWriter):
    ...
    # Get source selection (defaults for backward compatibility)
    ...
    # Build dynamic data sources section based on selected sources
    available_data_sources = build_data_sources_section(use_kdb=use_kdb, use_rag=use_rag, use_web=use_web)
    ...
    input = {
        ...
        "input": query_writer_instructions.format(
            topic=topic,
            report_organization=report_organization,
            number_of_queries=number_of_queries,
...
async def web_research(
    ...
    results = await asyncio.gather(*[
        process_single_query(query, config, writer, collection, llm, search_web, use_kdb=use_kdb)
