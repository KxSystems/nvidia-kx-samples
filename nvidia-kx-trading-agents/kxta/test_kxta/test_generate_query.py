...
from kxta.functions.generate_queries import KXTAGenerateQueriesConfig
from kxta.schema import GenerateQueryStateInput, GenerateQueryStateOutput, GeneratedQuery
...
from kxta.functions import artifact_qa, generate_summary, generate_queries
from kxta import register
...
# Integration test: builds the full workflow from configs/config.yml and runs it against
# live NIM LLMs (+ RAG). Skipped by default so the unit pass stays green offline.
#   RUN_INTEGRATION_TESTS=1 uv run pytest kxta/test_kxta/test_generate_query.py
pytestmark = pytest.mark.skipif(
    not os.getenv("RUN_INTEGRATION_TESTS"),
    reason="integration: requires live NIM LLMs + RAG; set RUN_INTEGRATION_TESTS=1 to enable",
)
