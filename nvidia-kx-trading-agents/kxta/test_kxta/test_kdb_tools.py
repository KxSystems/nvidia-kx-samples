...
from kxta.kdb_tools_nat import (
    is_kdb_query,
    KDB_KEYWORDS,
    KDB_ENABLED,
)
from kxta.search_utils import classify_query_type
...
class TestQueryClassification:
    ...
    async def test_classify_financial_query(self):
        ...
        with patch('kxta.search_utils.KDB_ENABLED', True):
    ...
    async def test_kdb_disabled_returns_rag(self):
        ...
        with patch('kxta.search_utils.KDB_ENABLED', False):
            query_type = await classify_query_type("What is AAPL stock price?")
            assert query_type == "rag"
...
class TestKDBNATClient:
    ...
    async def test_kdb_disabled_returns_empty(self):
        ...
        from kxta.kdb_tools_nat import search_kdb_nat

        with patch('kxta.kdb_tools_nat.KDB_ENABLED', False):
            answer, citation, record_count = await search_kdb_nat(
                "test query",
    ...
    async def test_intelligent_query_requires_llm(self):
        ...
        from kxta.kdb_tools_nat import KDBNATClient
        ...
        # With the mocked connection state set above, the client exposes its
        # discovered tools. (KDBNATClient connects inline via streamablehttp_client;
        # there is no _ensure_connected method to patch.)
