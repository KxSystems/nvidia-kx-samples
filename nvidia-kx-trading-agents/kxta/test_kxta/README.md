...
```bash
uv run pytest test_kxta/test_configmap_matches_config.py -s
```

This test validates that the Helm configmap template maintains the same structure as the reference configuration. It:
1. Compares the structure of `deploy/helm/kxta/templates/configmap.yaml` with `configs/config.yml`
...
```bash
uv run pytest test_kxta/test_generate_query.py -s
```
