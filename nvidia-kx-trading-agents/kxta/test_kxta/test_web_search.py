...
from kxta.schema import ConfigSchema, GeneratedQuery
from kxta.schema import KXTAState
from kxta.functions.generate_summary import GenerateSummaryStateInput, KXTAGenerateSummaryConfig

# Integration test: exercises web_research end-to-end, including the relevancy judge,
# which needs a live LLM (the mock only stands in for RAG). Skipped by default offline.
#   RUN_INTEGRATION_TESTS=1 uv run pytest kxta/test_kxta/test_web_search.py
pytestmark = pytest.mark.skipif(
    not os.getenv("RUN_INTEGRATION_TESTS"),
    reason="integration: relevancy judge needs a live LLM; set RUN_INTEGRATION_TESTS=1 to enable",
)
...
async def test_web_research_relevant(mock_rag_relevant, reasoning_llm, capsys):
    ...
    kxta_config = KXTAGenerateSummaryConfig(
        rag_url=url
    )
    ...
    user_input_passed_as_langchain_config = ConfigSchema(
        ...
        rag_url=kxta_config.rag_url,
        search_web=generate_summary_user_post_input.search_web
    )

    async with ClientSession() as session:
        
        with patch.object(RagSource, "is_available", return_value=True):
        ...
        kxta_stream_results = capsys.readouterr()
        print(kxta_stream_results.out)
        assert "citations" in result
        citations = result["citations"] 
        citation_list = citations.split("\n")
        assert len(citation_list) == 24 # based on the rag_response_relevant.json file (merged source formatting)
        assert "web_research_results" in result
        assert "{'score': 'yes'}" in kxta_stream_results.out  
...
async def test_web_research_not_relevant(mock_rag_not_relevant, reasoning_llm, capsys):
    ...
    async with ClientSession() as session:
        ...
        assert "citations" in result
        assert "web_research_results" in result
        assert "{'score': 'no'}" in kxta_stream_results.out  