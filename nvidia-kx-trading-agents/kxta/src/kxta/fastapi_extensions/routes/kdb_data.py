...
# Direct q-IPC write path (PyKX, unlicensed) — the loader writes here, not via the MCP.
from kxta.kdb_direct_write import KXTA_OWNED_TABLES
from kxta.kdb_direct_write import assert_kxta_owned
from kxta.kdb_direct_write import kdb_exec
from kxta.kdb_direct_write import kdb_insert

# Import MCP client for KDB+ integration (read-side: schema refresh so the agent sees new tables)
try:
    from kxta.kdb_tools_nat import KDB_ENABLED
    from kxta.kdb_tools_nat import get_kdb_nat_client
except ImportError:
    KDB_ENABLED = False
...
class LoadPublicDataRequest(BaseModel):
    ...
    data_types: List[str] = Field(default=["fundamentals", "news"],
...
def generate_synthetic_trades(daily_data: list, trades_per_day: int = 50) -> list:
    ...
    for day in daily_data:
        ...
        for i in range(trades_per_day):
            ...
            trades.append({"date": date_str, "time": trade_time, "sym": sym, "price": round(price, 2), "size": size})
    ...
    try:
        ...
        for article in news_list[:limit]:
            ...
            if pub_date_str:
                ...
                pub_date = datetime.fromtimestamp(pub_time).strftime(
                    "%Y.%m.%d") if pub_time else datetime.now().strftime("%Y.%m.%d")
                pub_timestamp = datetime.fromtimestamp(pub_time).strftime(
                    "%H:%M:%S.000") if pub_time else "00:00:00.000"
...
async def create_public_data_tables() -> dict:
    ...
    try:
        ...
        tables = {
            "fundamentals":
                "`fundamentals set ([]date:`date$();sym:`$();name:();sector:();industry:();market_cap:0f;pe_ratio:0f;forward_pe:0f;peg_ratio:0f;price_to_book:0f;eps:0f;dividend_yield:0f;beta:0f;fifty_two_week_high:0f;fifty_two_week_low:0f;avg_volume:0j;shares_outstanding:0j)",
            "news":
                "`news set ([]date:`date$();time:`time$();sym:`$();title:();summary:();publisher:();link:();news_type:())",
            "recommendations":
                "`recommendations set ([]date:`date$();sym:`$();firm:();to_grade:();from_grade:();action:())",
        }
        ...
        for table_name, create_query in tables.items():
            try:
                ...
                check_result = await kdb_exec(check_query)
                ...
                if check_result.get("error") or check_result.get("isError") or "error" in str(check_result).lower():
                    ...
                    result = await kdb_exec(create_query)
...
async def insert_public_data(data_type: str, data: list) -> dict:
    ...
    try:
        ...
        # Typed bulk insert: row values become typed q column vectors client-side —
        # no q code is built from the data, so no quote-escaping is needed.
        if data_type not in ("fundamentals", "news", "recommendations"):
            return {"success": False, "message": f"Unknown data type '{data_type}'", "rows_inserted": 0}

        result = await kdb_insert(data_type, data)
        rows_inserted = result.get("rows", 0)
        if result.get("isError"):
            logger.warning(f"Bulk insert failed for {data_type}: {result.get('error')}")
            return {"success": False, "message": str(result.get("error")), "rows_inserted": 0}
...
async def generate_public_data_stream(request: LoadPublicDataRequest) -> AsyncGenerator[str, None]:
    ...
    yield await sse_event("progress", {
        "phase": "Starting public data import...", "overall_progress": 0, "status": "loading"
    })

    # Create tables if needed
    yield await sse_event("progress", {"phase": "Preparing tables...", "overall_progress": 5, "status": "loading"})
...
async def insert_data_to_kdb(symbol: str, data: dict, progress_callback=None) -> dict:
    ...
    if not KDB_ENABLED or get_kdb_nat_client is None:
        ...
        return {"success": True, "message": f"Mock insertion for {symbol} (KDB disabled)", "rows_inserted": total_rows}

    try:
        client = get_kdb_nat_client()
        if client is None:
            return {"success": False, "message": "KDB client not available", "rows_inserted": 0}

        await client.initialize()
        rows_inserted = 0
        batch_size = 100  # Insert 100 rows per batch

        # Typed bulk inserts via PyKX: row values become typed q column vectors
        # client-side — no q insert statements are built from the data.
        for table, rows in (("daily", daily_rows), ("trade", trade_rows), ("quote", quote_rows)):
            for i in range(0, len(rows), batch_size):
                batch = rows[i:i + batch_size]
                result = await kdb_insert(table, batch)
                if not result.get("isError"):
                    rows_inserted += result.get("rows", len(batch))
                else:
                    logger.warning(f"Batch insert failed for {table} {symbol} batch {i}: {result.get('error')}")
                if progress_callback:
                    await progress_callback(rows_inserted, total_rows)

        return {
            "success": True, "message": f"Inserted {rows_inserted} rows for {symbol}", "rows_inserted": rows_inserted
        }

    except Exception as e:
        logger.error(f"Error inserting data to KDB for {symbol}: {e}")
        return {"success": False, "message": f"Error: {str(e)}", "rows_inserted": 0}
...
async def clear_kdb_tables() -> dict:
    ...
    if not KDB_ENABLED or get_kdb_nat_client is None:
        ...
        return {"success": True, "message": "Mock table clear (KDB disabled)"}

    try:
        client = get_kdb_nat_client()
        if client is None:
            return {"success": False, "message": "KDB client not available"}

        await client.initialize()

        tables_cleared = []

        # Clear each table. Hardcoded to KXTA-owned tick tables only — never enumerate
        # tables[] — so co-located data (e.g. RAG collections) is never touched.
        # `set` the empty typed schema: this CREATES the table if it doesn't exist (e.g. when
        # co-locating in a KDB-X that only had RAG tables) and CLEARS it if it does. The column
        # order/types must match the batched inserts below.
        tick_schemas = {
            "daily": "`daily set ([]date:`date$();sym:`$();open:0f;high:0f;low:0f;close:0f;volume:0j)",
            "trade": "`trade set ([]date:`date$();time:`time$();sym:`$();price:0f;size:0j)",
            "quote": "`quote set ([]date:`date$();time:`time$();sym:`$();bid:0f;ask:0f;bsize:0j;asize:0j)",
        }
        for table in ["daily", "trade", "quote"]:
            assert_kxta_owned(table)  # belt-and-suspenders guard
            result = await kdb_exec(tick_schemas[table])
            if not result.get("error"):
                tables_cleared.append(table)
            else:
                logger.warning(f"Prepare table {table} failed: {result.get('error')}")

        if tables_cleared:
            return {"success": True, "message": f"Cleared tables: {', '.join(tables_cleared)}"}
        else:
            return {"success": True, "message": "Tables prepared for loading"}
...
async def generate_progress_stream(request: LoadHistoricalDataRequest,
                                   job_id: Optional[str] = None,
                                   resume_from: Optional[List[str]] = None) -> AsyncGenerator[str, None]:
    """
    ...
    if not is_resume:
        ...
        if not clear_result["success"]:
            ...
            job.last_update = datetime.now().isoformat()
            save_job_state(job)
            yield await sse_event("error", {"job_id": job_id, "message": job.error})
            return
    ...
    for i, symbol in enumerate(symbols):
        ...
        if is_job_cancelled(job_id):
            ...
            job.last_update = datetime.now().isoformat()
            save_job_state(job)
            yield await sse_event("error", {"job_id": job_id, "message": "Job cancelled by user"})
            return
            ...
            yield await sse_event(
                "progress",
                {
                    "job_id":
                        job_id,
                    "phase":
                        f"Generated data for {symbol}",
                    "overall_progress":
                        int(5 + ((i + 1) / total_symbols) * 35),
                    "symbol":
                        symbol,
                    "status":
                        "complete",
                    "message":
                        f"{result['daily_rows']} daily + {result['trade_rows']} trades + {result['quote_rows']} quotes",
                    "rows_loaded":
                        total_rows
                })
        else:
...
async def add_kdb_data_routes(app: FastAPI):
    ...
    if not KDB_MCP_INTERNAL:
        logger.info("KDB data loader routes DISABLED (KDB_MCP_INTERNAL=false). "
    ...
    async def load_historical_data(request: LoadHistoricalDataRequest):
        ...
        return StreamingResponse(generate_progress_stream(request),
    ...
    async def retry_job(job_id: str):
        ...
        if job.status == "running":
            raise HTTPException(status_code=400, detail="Cannot retry a running job. Cancel it first.")

        if job.status == "completed":
            raise HTTPException(status_code=400, detail="Job already completed successfully. Start a new job instead.")
        ...
        request = LoadHistoricalDataRequest(symbols=job.symbols, start_date=job.start_date, end_date=job.end_date)

        # Determine remaining symbols
        remaining_symbols = [s for s in job.symbols if s not in job.completed_symbols]

        return StreamingResponse(generate_progress_stream(request, job_id=new_job_id,
    ...
    app.add_api_route("/kdb/load-historical",
                      load_historical_data,
                      methods=["POST"],
                      tags=["kdb-endpoints"],
                      summary="Load historical stock data into KDB+ (DEMO - clears all tables)")

    app.add_api_route("/kdb/jobs",
                      list_jobs,
                      methods=["GET"],
                      tags=["kdb-endpoints"],
                      summary="List recent data loading jobs")

    app.add_api_route("/kdb/jobs/active",
                      get_active,
                      methods=["GET"],
                      tags=["kdb-endpoints"],
                      summary="Get currently running job")

    app.add_api_route("/kdb/jobs/{job_id}",
                      get_job,
                      methods=["GET"],
                      tags=["kdb-endpoints"],
                      summary="Get status of a specific job")

    app.add_api_route("/kdb/jobs/{job_id}/cancel",
                      cancel_job,
                      methods=["POST"],
                      tags=["kdb-endpoints"],
                      summary="Cancel a running job")

    app.add_api_route("/kdb/jobs/{job_id}/retry",
                      retry_job,
                      methods=["POST"],
                      tags=["kdb-endpoints"],
    ...
    async def load_public_data(request: LoadPublicDataRequest):
        ...
        return StreamingResponse(generate_public_data_stream(request),
                                 media_type="text/event-stream",
                                 headers={
                                     "Cache-Control": "no-cache",
                                     "Connection": "keep-alive",
    ...
    app.add_api_route("/kdb/load-public",
                      load_public_data,
                      methods=["POST"],
                      tags=["kdb-endpoints"],
