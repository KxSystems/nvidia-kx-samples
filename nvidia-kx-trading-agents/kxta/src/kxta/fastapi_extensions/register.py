...
from .routes.kdb_data import add_kdb_data_routes
from .routes.kdb_chat import add_kdb_chat_routes
from .routes.kdb_docs_settings import add_kdb_docs_settings_routes
from .routes.kdb_tables_settings import add_kdb_tables_settings_routes
from .routes.sec_ingest import add_sec_ingest_routes
from .routes.source_agents import add_source_agents_routes


class APIExtensionsConfig(FastApiFrontEndConfig, name="kxta_frontend"):
...
class APIExtensionsWorker(FastApiFrontEndPluginWorker):
    ...
    async def add_routes(self, app: FastAPI, builder: WorkflowBuilder):
        ...
        # Add KDB+ data loading routes
        await add_kdb_data_routes(app)

        # Add KDB+ chat routes
        await add_kdb_chat_routes(app)

        # Add source agents availability route
        await add_source_agents_routes(app)

        # Add SEC filings -> RAG ingestion routes
        await add_sec_ingest_routes(app, rag_ingest_url)

        # Add kdb_docs collection-selection settings routes
        await add_kdb_docs_settings_routes(app, rag_ingest_url)

        # Add kdb table-selection settings routes
        await add_kdb_tables_settings_routes(app)
