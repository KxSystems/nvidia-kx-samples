...
@dataclass
class KXTAHallucination(MetricWithLLM, SingleTurnMetric):
    """
...
"""
    ...
    async def _single_turn_ascore(self, sample: SingleTurnSample, callbacks: Callbacks) -> float:
        ...
        # Log rationales if hallucinations detected
        if score_a > 0:
            logger.debug("Hallucination rationale (Template A):\n%s", self.rationale_a)
        if score_b > 0:
            logger.debug("Hallucination rationale (Template B):\n%s", self.rationale_b)

        # Return average of both template scores
        return (score_a + score_b) / 2.0


def prepare_hallucination_data_from_kxta_example(example_data: dict) -> tuple:
    """
    Extract and prepare hallucination evaluation data from KXTA example exactly like the notebook.
    
    Args:
        example_data: KXTA example dictionary
...
class HallucinationEvaluator:
    ...
    def __init__(self, llm: BaseLanguageModel, max_concurrency: int = 4, output_dir: str = None):
        ...
        self.output_dir = output_dir or ".tmp/kxta"
    ...
    async def evaluate_item(self, item: EvalInputItem) -> EvalOutputItem:
        ...
        scorer = KXTAHallucination(llm=self.llm)
