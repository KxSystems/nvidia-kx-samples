...
from kxta.eval.generator_register import KXTAGeneratorBase
from kxta.eval.generator_register import register_generator
...
class KXTAFullGenerator(KXTAGeneratorBase):
    """
    Full implementation of the NVIDIA Trading Agents Blueprint generator that uses the complete NVIDIA Trading Agents Blueprint workflow.
    This integrates with the actual NVIDIA Trading Agents Blueprint pipeline including query generation, research, and report generation.
    """
    ...
    def setup_generator(self):
        '''Setup the full generator with NVIDIA Trading Agents Blueprint workflow functions.'''
        logger.info("Setting up full NVIDIA Trading Agents Blueprint generation mechanism.")

        # Get the NVIDIA Trading Agents Blueprint workflow functions from the builder
        ...
        logger.info("Successfully loaded NVIDIA Trading Agents Blueprint workflow functions")
    ...
    async def extract_evaluation_outputs(self,
        ...
        if verbose:
            # from kxta.eval.generators.extraction_utils import debug_report_structure
    ...
    async def generate_fn(self, kxta_input: AIResearcherEvalInput) -> AIResearcherEvalOutput:
        '''Full generate function that runs the complete KXTA research pipeline.'''
        ...
        if verbose:
            logger.info("=" * 80)
            logger.info("Starting KXTA generation for instance: %s", kxta_input.id)
            logger.info("  topic: %s", kxta_input.topic)
            logger.info("  report_organization: %s", kxta_input.report_organization)
            logger.info("  rag_collection: %s", kxta_input.rag_collection)
            logger.info("  search_web: %s", kxta_input.search_web)
            logger.info("  num_queries: %s", kxta_input.num_queries)
            logger.info("  reflection_count: %s", kxta_input.reflection_count)
        else:
            logger.info("Processing instance: %s", kxta_input.id)

        # Pre-process the input
        kxta_input = await self.pre_process(kxta_input)

        # Handle LLM name - use from input if provided, otherwise use from config
        llm_name = kxta_input.llm_name
        if not llm_name:
        ...
        query_input = GenerateQueryStateInput(
            topic=kxta_input.topic,
            report_organization=kxta_input.report_organization,
            num_queries=kxta_input.num_queries,
        ...
        summary_input = GenerateSummaryStateInput(
            ...
            search_web=kxta_input.search_web,
            rag_collection=kxta_input.rag_collection,
            llm_name=llm_name,  # Use the determined llm_name
            reflection_count=kxta_input.reflection_count)
        ...
        output = await self.extract_evaluation_outputs(kxta_input, query_result, summary_result)
        ...
        if verbose:
            ...
            logger.info("=" * 80)
            logger.info("Completed processing instance: %s", kxta_input.id)
        else:
            logger.info("Completed instance: %s", kxta_input.id)
