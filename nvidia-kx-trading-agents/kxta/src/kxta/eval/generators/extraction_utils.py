...
"""Extraction utilities for KXTA evaluation workflow."""
...
def get_bear_token():
    ...
    url = os.environ.get("NV_OAUTH_URL", "https://prod.api.nvidia.com/oauth/api/v1/ssa/default/token")
...
async def _get_llm_response_structured(
    ...
    if model.startswith("gpt-"):
        try:
            ...
            api_base = os.environ.get("NV_AZURE_API_BASE", "https://prod.api.nvidia.com/llm/v1/azure")
...
def split_report_and_citations(final_report: str) -> Tuple[str, str]:
    ...
    # Try multiple citation prefixes, including KXTA format with dashes
    citation_prefixes = [
        "## Sources---",  # KXTA format with dashes
...
def parse_kxta_sources(citation_section: str) -> Dict[int, str]:
    """
    Parse KXTA citation section to extract source number and content.
    
    Args:
        citation_section: The citation section from KXTA report
...
async def pair_facts_with_kxta_sources(facts: List[str],
                                       citation_section: str,
                                       llm: str,
                                       verbose: bool = False) -> List[Tuple[str, List[int]]]:
    """
    Pair facts with KXTA sources when no inline citations exist.
    ...
    if not facts:
        if verbose:
            logger.info("No facts provided for KXTA source matching")
        return []

    if verbose:
        logger.info(f"Starting KXTA source matching for {len(facts)} facts")
        logger.info(f"Citation section length: {len(citation_section)} characters")

    # Parse sources from citation section
    sources = parse_kxta_sources(citation_section)
...
"""
    ...
    if verbose:
        logger.info(f"KXTA source matching completed:")
...
async def pair_facts_with_citations(final_report: str,
    ...
    if not citation_matches:
        if verbose:
            ...
            logger.warning("Attempting to use KXTA source format instead...")

        # Try to extract citation section and use KXTA format
        summary_section, citation_section = split_report_and_citations(final_report)

        if citation_section:
            if verbose:
                logger.info("Found citation section, using KXTA source matching")
            return await pair_facts_with_kxta_sources(facts, citation_section, llm, verbose)
        else:
            if verbose:
...
async def batch_pair_facts_with_citations(final_report: str,
    ...
    if not citation_matches:
        ...
        if citation_section:
            ...
            # Use KXTA source matching
            try:
                result = await pair_facts_with_kxta_sources(facts, citation_section, llm, verbose)
                if verbose:
                    facts_with_citations = sum(1 for _, citations in result if citations)
                    logger.info(
                        f"KXTA source matching completed: {facts_with_citations}/{len(result)} facts have citations")
                return result
            except Exception as e:
                if verbose:
                    logger.error(f"KXTA source matching failed: {str(e)}")
                    logger.exception("Full exception details:")
    ...
    try:
        ...
        if isinstance(raw_result, dict) and "fact_citations" in raw_result:
            ...
            if verbose:
                ...
                logger.warning("Falling back to KXTA source matching...")

            # Fallback to KXTA source matching when inline citation processing fails
            summary_section, citation_section = split_report_and_citations(final_report)

            if citation_section:
                if verbose:
                    logger.info("Found citation section, using KXTA source matching as fallback")
                    logger.info(f"Citation section preview: {citation_section[:200]}...")
                ...
                try:
                    ...
                    if verbose:
                        facts_with_citations = sum(1 for _, citations in result if citations)
                        logger.info(
                            f"KXTA source matching fallback completed: {facts_with_citations}/{len(result)} facts have citations"
                        )
                    return result
                except Exception as e:
                    if verbose:
                        logger.error(f"KXTA source matching fallback failed: {str(e)}")
                        logger.exception("Full exception details:")
                    # Fall back to empty citations
                    return [(fact, []) for fact in facts]
            else:
        ...
        # If no facts have citations despite finding inline citations, try KXTA fallback
        if facts_with_citations == 0 and citation_matches:
            if verbose:
                logger.warning(
                    "No facts were successfully paired with inline citations. Trying KXTA source matching as fallback..."
                )
            ...
            if citation_section:
                if verbose:
                    logger.info("Found citation section, using KXTA source matching as secondary fallback")
                    logger.info(f"Citation section preview: {citation_section[:200]}...")
                ...
                try:
                    ...
                    if verbose:
                        ...
                        logger.info(
                            f"KXTA source matching secondary fallback completed: {facts_with_citations_fallback}/{len(result)} facts have citations"
                        )
                    return result
                except Exception as e:
                    if verbose:
                        logger.error(f"KXTA source matching secondary fallback failed: {str(e)}")
                        logger.exception("Full exception details:")
        ...
        # Try KXTA source matching as fallback before individual processing
        if verbose:
            logger.info("Trying KXTA source matching as exception fallback...")

        summary_section, citation_section = split_report_and_citations(final_report)

        if citation_section:
            if verbose:
                logger.info("Found citation section, using KXTA source matching as exception fallback")
                logger.info(f"Citation section preview: {citation_section[:200]}...")
            ...
            try:
                ...
                if verbose:
                    facts_with_citations = sum(1 for _, citations in result if citations)
                    logger.info(
                        f"KXTA source matching exception fallback completed: {facts_with_citations}/{len(result)} facts have citations"
                    )
                return result
            except Exception as e2:
                if verbose:
                    logger.error(f"KXTA source matching exception fallback failed: {str(e2)}")
                    logger.exception("Full exception details:")
