...
from kxta.functions import generate_summary, generate_queries, artifact_qa
from nat.builder.framework_enum import LLMFrameworkEnum
from nat.plugins.langchain import register
from kxta.fastapi_extensions.register import APIExtensionsConfig
...
async def default_collections(config: DefaultCollectionsConfig, builder: Builder):
    """
    Returns information about the example collections used by the NVIDIA Trading Agents Blueprint demo frontend
    """
    ...
    yield FunctionInfo.from_fn(
        ...
        description="Information about the example collections used by the NVIDIA Trading Agents Blueprint demo frontend"
    )
...
async def health_check(config: HealthCheckConfig, builder: Builder):
    """
    Health check for the NVIDIA Trading Agents Blueprint backend service
    """

    async def _health_check(request: None = None) -> dict:
        return {"status": "OK"}

    yield FunctionInfo.from_fn(_health_check,
                               description="Health check for the NVIDIA Trading Agents Blueprint service")
...
# are used in the NVIDIA Trading Agents Blueprint demo frontend
# the ai_researcher workflow is not used in the NVIDIA Trading Agents Blueprint demo frontend
...
class AIResearcherWorkflowConfig(FunctionBaseConfig, name="ai_researcher"):
    ...
    This workflow is not used in the NVIDIA Trading Agents Blueprint demo frontend
    """
    rag_url: str = ""
