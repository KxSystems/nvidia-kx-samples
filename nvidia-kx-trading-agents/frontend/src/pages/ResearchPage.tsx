...
export function ResearchPage() {
  ...
  // Guard: research without any enabled agent silently produces an
  // evidence-free report. Block it here (deep links bypass the Home guard).
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      {/* No agents enabled — research would produce an evidence-free report */}
      {noSources && (
        <div className="flex items-center gap-3 bg-amber-50 border border-amber-300 rounded-lg p-4" role="alert">
          <AlertCircle className="w-5 h-5 text-amber-600 flex-shrink-0" />
          <p className="text-sm text-amber-900">
            <span className="font-semibold text-amber-800">No agents enabled.</span>{' '}
            Research needs at least one data source —{' '}
            <Link to="/" className="text-blue-600 underline hover:no-underline">
              select agents on the Home page
            </Link>{' '}
            first.
          </p>
        </div>
              ...
              >
                <div
                  className={clsx(
                    ...
                    isCurrent && 'bg-nvidia-green text-white',
                    isCompleted && !isCurrent && 'bg-nvidia-gray-600 text-nvidia-gray-50',
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          {(isThinking || thinkingContent || queries.length > 0) && topic && (
            <div className="space-y-4">
              <div>
                ...
                <p className="text-nvidia-gray-50">{topic}</p>
              </div>
              {reportOrganization && (
                <div>
                  <h3 className="text-sm font-medium text-nvidia-gray-400 uppercase tracking-wide mb-1">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-semibold text-nvidia-gray-50">Generating Research Plan</h2>
          ...
          {queries.length > 0 && (
            <div ref={queryListRef}>
              <QueryList onExecutePlan={handleExecutePlan} executeDisabled={noSources} />
            </div>
          )}
        </div>
      )}
