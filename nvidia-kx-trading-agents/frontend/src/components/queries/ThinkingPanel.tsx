...
export function ThinkingPanel({ content, isThinking, isStreaming = false, modelName = 'LLAMA NEMOTRON SUPER' }: ThinkingPanelProps) {
  ...
  // Collapsed by default: the raw reasoning trace is a debug view, not the product.
  // A one-line live preview keeps the panel alive; expand to watch the full trace.
  ...
  // Last non-empty line of the cleaned trace — the collapsed one-line live preview.
  const lastLine = cleanContent.split('\n').filter((l) => l.trim()).pop() ?? '';
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        ...
        className="flex items-center justify-between cursor-pointer rounded focus:outline-none focus-visible:ring-2 focus-visible:ring-nvidia-green"
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h2 className="text-xl font-semibold text-nvidia-gray-50">
              {showAsActive ? 'Analyzing the market question...' : 'Reasoning Complete'}
            </h2>
            {!isExpanded && showAsActive && lastLine ? (
              <p className="text-sm text-nvidia-gray-400 mt-1 truncate max-w-xl" title={lastLine}>
                {lastLine}
              </p>
            ) : (
        ...
        <span className="flex items-center gap-1 p-2 text-nvidia-gray-400 hover:text-nvidia-gray-50 transition-colors shrink-0">
          <span className="text-xs hidden sm:inline">{isExpanded ? 'Hide reasoning' : 'Show reasoning'}</span>
