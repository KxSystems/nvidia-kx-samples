...
export function KDBChatMessage({ message }: KDBChatMessageProps) {
  ...
  return (
    ...
    >
      ...
      >
        {isUser ? (
          <User className="w-4 h-4 text-nvidia-gray-50" />
        ) : message.isThinking ? (
      ...
      <div
        className={clsx(
          ...
          isUser
            ? 'bg-nvidia-green text-white rounded-tr-sm'
            : 'bg-nvidia-gray-700 text-nvidia-gray-50 rounded-tl-sm'
        )}
      >
        {/* Main content */}
        <p className={clsx(
        ...
        <span
          className={clsx(
            'text-xs mt-2 block',
            isUser ? 'text-white/60' : 'text-nvidia-gray-400'
          )}
        >
