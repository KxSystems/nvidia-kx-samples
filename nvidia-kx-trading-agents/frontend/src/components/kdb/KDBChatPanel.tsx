    ...
    <Card className="flex flex-col h-[600px]">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-nvidia-gray-700">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-nvidia-gray-50">KDB-X Data Chat</h2>
      ...
      {/* Error banner */}
      {error && (
        <div className="flex items-center gap-2 p-3 mt-4 rounded-lg bg-red-500/10 border border-red-500/30 text-red-600">
      ...
      {status && !status.connected && (
        <div className="flex items-center gap-2 p-3 mt-4 rounded-lg bg-yellow-500/10 border border-yellow-500/30 text-yellow-600">
          <AlertCircle className="w-4 h-4 flex-shrink-0" />
          <div className="text-sm">
            <p className="font-medium">KDB+ is not connected</p>
            <p className="text-xs mt-1 text-yellow-600/80">{status.message}</p>
          </div>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto py-4 space-y-4 min-h-0">
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center">
            <Database className="w-12 h-12 text-nvidia-gray-500 mb-4" />
              ...
              <div className="mt-4 grid gap-2 w-full max-w-md">
                {EXAMPLE_QUERIES.map((query, i) => (
                  <button
                    ...
                    className="text-left px-3 py-2 rounded-lg bg-nvidia-gray-800 hover:bg-nvidia-gray-700 text-sm text-nvidia-gray-300 hover:text-nvidia-gray-50 transition-colors"
                  >
                    {query}
                  </button>
                ))}
        ...
        <div className="flex gap-3">
          <div className="flex-1 relative">
            <textarea
              ...
              className="w-full px-4 py-3 bg-nvidia-gray-800 border border-nvidia-gray-600 rounded-xl text-nvidia-gray-50 placeholder-nvidia-gray-500 focus:outline-none focus:border-nvidia-green focus:ring-1 focus:ring-nvidia-green disabled:opacity-50 disabled:cursor-not-allowed resize-none"
              rows={1}
