...
# 3. Deploy
helm upgrade --install kdb-mcp . -n kxta -f my-values.yaml
```
...
## Integrating with KXTA Backend

After installing this chart, configure your KXTA backend to use the KDB MCP server:

### Using ConfigMap (recommended)

The chart creates a ConfigMap with the MCP endpoint configuration. Reference it in your KXTA deployment:

```yaml
# In your kxta values.yaml
backendEnvVars:
  KDB_ENABLED: "true"
...
### Check pod status
```bash
kubectl get pods -n kxta -l app.kubernetes.io/name=kdb-x-mcp-server
```

### View MCP Server logs
```bash
kubectl logs -n kxta -l app.kubernetes.io/name=kdb-x-mcp-server
```

### View KDB-X Database logs
```bash
kubectl logs -n kxta -l app.kubernetes.io/component=kdbx-database
```

### View KDB-X init container logs (runtime mode)
```bash
kubectl logs -n kxta <pod-name> -c install-kdbx
```

### Test MCP connectivity
```bash
kubectl port-forward -n kxta svc/kdb-mcp-kdb-x-mcp-server 8000:8000
curl http://localhost:8000/health
```

### Test KDB-X connectivity
```bash
kubectl port-forward -n kxta svc/kdb-mcp-kdb-x-mcp-server-kdbx 5000:5000
```
