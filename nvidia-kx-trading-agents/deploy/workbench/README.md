...
1. On the **Project Dashboard**, select the ``kxta-no-gpu`` compose profile from the dropdown under the **Compose** section.

   * Alternatively, select the ``kxta-gpu`` option to start an additional LLM NIM for report generation; 2x GPUs required.
...
1. When the compose services are ready, you can access the frontend on the IP address, eg. ``http://<ip_addr>:3000``.
   ...
   * If ``ERR_CONNECTION_REFUSED``, try restarting the frontend container: **Environment** > **Compose** > **kxta-frontend** > **restart**.
