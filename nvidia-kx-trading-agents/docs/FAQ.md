...
The demo web application allows you to upload 10 files at a time. This process can be repeated to add additional files to a collection. Alternatively, you can bulk upload files. To do so, see the example utility `data/zip_to_collection.py`. To run this utility create a zip file containing your desired files, and then run the utility either directly in Python or by building and running the docker container, eg.
    ...
    # assuming you have deployed RAG and KXTA via docker-compose, otherwise adjust the env vars accordingly
    docker run \
...
Yes. The NVIDIA RAG blueprint is optional — the `rag` agent is gated on a reachability probe of `RAG_SERVER_URL`, so if RAG isn't deployed it just shows as "not deployed" and the backend still runs. A standalone deployment is **KDB-X + the external market-intelligence agents (`kdb`, `kdb_pit`, `market_data`, `news_headlines`, `fundamentals`, `sec_filings`, `macro_economic`, `web_search`, `onetick`) + the two LLMs**. The only hard requirement RAG normally satisfies is hosting the reasoning/writing LLMs, which you instead supply via cloud endpoints or the `nim-llm`/`nim-llm-nemotron` subcharts. You lose document retrieval and RAG-based ingestion (upload UI, demo collections, `/sec/ingest`, populating `kdb_docs`). See [Standalone (no RAG)](./configuration-reference.md#standalone-no-rag).

## How does the Assistant Decide Which Data Source to Use?

Routing is per query. The reasoning model tags each planned query with a preferred source; the router honors that tag, falling back to keyword matching and then semantic similarity (embedding NIM) for `auto`. The chosen agent(s) run in parallel, results are scored by a reranker NIM (or an LLM judge), and on a relevancy miss the orchestrator reroutes to up to two fallback sources before finally trying web search. The live `GET /source_agents` endpoint (and the UI agent picker) shows which agents are currently available and why.

## How many GPUs do I Need? Can I Deploy with Fewer GPUs?

The blueprint hardware requirements for the default Docker compose deployment with local NVIDIA NIM microservices are outlined in the [README](../README.md#hardware-requirements). Fewer GPUs can be used by deploying the NVIDIA RAG blueprint following [these guidelines](https://github.com/KxSystems/nvidia-kx-samples/blob/main/KX-nvidia-rag-blueprint/docs/support-matrix.md#hardware-requirements-docker).
...
The REST endpoints are created using the [NVIDIA NeMo Agent Toolkit](https://github.com/NVIDIA/NeMo-Agent-Toolkit). Each endpoint is defined as a function and frontend endpoint in the `configs/config.yml` configuration file. The docker compose entrypoint for the backend invokes the `nat serve` command which makes these endpoints available as REST APIs. The core research functions (`generate_query`, `generate_summary`, `artifact_qa`) are registered with the NeMo Agent Toolkit in the `register.py` file, with source in the `functions` directory. Additional FastAPI routes — `GET /source_agents`, `GET/PUT /settings/kdb-docs`, `GET/PUT /settings/kdb-tables`, `POST /sec/ingest`, `GET /sec/collections`, `/kdb/chat`, and `/kdb/load-historical` — are registered as FastAPI extensions (see `fastapi_extensions/routes/`). The full list is in [docs/rest-api.md](./rest-api.md).
...
Update the file `kxta/src/kxta/constants.py` to include a list of approved domains. When this list is configured, the search function will only search these domains for web queries.

## How do I Increase Timeouts?

The report generation is designed to be robust to intermittent timeouts in LLM calls, RAG search, or web search. In these cases, the frontend web application will notify users about the timeout but proceed with report creation. The backend service log will also note the timeout. To increase the timeout, update the value `ASYNC_TIMEOUT` in the file `kxta/src/kxta/constants.py`.
