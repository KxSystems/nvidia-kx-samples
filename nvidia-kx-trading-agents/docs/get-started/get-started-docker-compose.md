...
Optionally enable additional source agents and NIM enhancements — each is
enable-when-present, so unset values simply leave that agent/feature off:

```bash
# Source-agent providers (optional)
export ALPHAVANTAGE_API_KEY=...   # market_data + news_headlines agents
export FRED_API_KEY=...           # macro_economic agent
export FIRECRAWL_API_KEY=...      # web_search (falls back to Tavily)
export SEC_EDGAR_EMAIL=you@org.com  # sec_filings identity (not a secret)
# OneTick Cloud (optional): ONETICK_CLIENT_ID / ONETICK_CLIENT_SECRET

# NIM enhancements (optional — reuse RAG-cluster NIMs at no extra GPU cost)
export RERANKER_NIM_URL=...        # relevancy gate (else LLM judge)
export EMBEDDING_NIM_URL=...       # semantic routing/dedup + kdb_docs vectors
```

See the [Configuration Reference](../configuration-reference.md) for the full
list, and the [KDB-X section](#optional-kdb-x-financial-data-integration) below
for KDB-X (the `kdb`, `kdb_docs`, and `kdb_pit` agents).
...
```bash
export KXTA_LLM_MS_GPU_ID=3,4,5,6
```

For B200 system, run the following command:
```bash
export KXTA_LLM_MS_GPU_ID=3,4
```

Run the following to deploy the model:
```bash
docker compose -f deploy/compose/docker-compose.yaml --profile kxta-instruct-llm up -d
```
...
```
kxta-frontend                    Up 2 minutes
kxta-backend                     Up 2 minutes
```
...
```bash
docker compose -f deploy/compose/docker-compose.yaml --profile kxta-instruct-llm down
```
...
> **Self-hosted vs. cloud models:** the on-prem path above self-hosts the NVIDIA
> NIMs (local Instruct via `--profile kxta-instruct-llm`, Nemotron via RAG's
> `nim-llm-ms`, or a fully local Nemotron with `docker-compose-kx-local.yaml`).
> The section below instead points the `*_BASE_URL` variables at NVIDIA-hosted
> cloud endpoints. Either way it's the same `INSTRUCT_BASE_URL` / `NEMOTRON_BASE_URL`
> switch — see [Self-hosted vs. cloud NIMs](../configuration-reference.md#self-hosted-vs-cloud-nims-and-air-gap).
...
Edit the *Docker Compose file* located at `deploy/compose/docker-compose.yaml`.
  ...
  - [ ] Update the value `services.kxta-backend.environment.TAVILY_API_KEY` with your TAVILY API Key
  - [ ] If you have deployed RAG on a different server than KXTA, update the value `services.kxta-backend.environment.RAG_INGEST_URL` with the *public http IP address of the RAG ingestor service* such as `http://UPDATE-TO-YOUR-RAG-IP-SERVER:8082`. If you have deployed RAG using docker compose on the same server as KXTA, leave the default value.
...
## Optional: KDB-X Financial Data Integration

This adds KDB-X time-series database integration for real-time financial data queries.
...
Deploy everything including dedicated LLM NIMs (requires GPUs):
...
### Required Credentials for KDB-X

| Variable | Source | Purpose |
|----------|--------|---------|
| `NVIDIA_API_KEY` | [build.nvidia.com](https://build.nvidia.com) | LLM API access |
| `KDB_BEARER_TOKEN` | [portal.kx.com](https://portal.kx.com) → Profile → Tokens | KDB-X installer download |
| `KDB_LICENSE_B64` | [kx.com/kdb-personal-edition](https://kx.com/kdb-personal-edition/) | PyKX license (base64 encoded) |

### Verify KDB-X Integration

```bash
# Check all containers are running
docker ps --format "table {{.Names}}\t{{.Status}}"

# Test KDB connectivity
