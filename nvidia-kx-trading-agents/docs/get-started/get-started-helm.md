...
--set backendEnvVars.RAG_INGEST_URL=<INGESTOR_SERVER_URL> -n kxta
```

Optional source agents and NIM enhancements are configured the same way, via
`backendEnvVars` (each is enable-when-present). For example, to light up the
market-data/news, macro, and web-research agents and reuse the RAG cluster's
reranker/embedding NIMs:

```bash
helm install kxta kxta/ \
--set ngcApiSecret.password=$NGC_API_KEY \
--set tavilyApiSecret.password=$TAVILY_API_KEY \
--set backendEnvVars.ALPHAVANTAGE_API_KEY=<key> \
--set backendEnvVars.FRED_API_KEY=<key> \
--set backendEnvVars.FIRECRAWL_API_KEY=<key> \
--set backendEnvVars.RERANKER_NIM_URL=<reranker-nim-url> \
--set backendEnvVars.EMBEDDING_NIM_URL=<embedding-nim-url> -n kxta
```
...
(`nim-llm` subchart, `enabled: true`) and, by default, reaches the co-deployed RAG
blueprint's `nim-llm` for Nemotron. To self-host Nemotron in-chart too (e.g. a
standalone or air-gapped install), enable the second subchart and point Nemotron at it:
...
To use **cloud** models instead, override the base URLs and disable the subcharts:

```bash
--set nim-llm.enabled=false \
--set backendEnvVars.INSTRUCT_BASE_URL=https://integrate.api.nvidia.com/v1 \
--set backendEnvVars.NEMOTRON_BASE_URL=https://integrate.api.nvidia.com/v1
```

See [Self-hosted vs. cloud NIMs](../configuration-reference.md#self-hosted-vs-cloud-nims-and-air-gap).
...
Using the list of model profiles from the previous step, set the NIM_MODEL_PROFILE in the `nim-llm` section of the [values.yaml](../../deploy/helm/kxta/values.yaml). It is ideal to select one of the tensorrt_llm profiles for best performance. Here is an example of selecting one of these profiles for two H100 GPUs:
```
env:
...
```bash
kubectl get pods -n kxta
```

Response should look like this:
```
NAME                                      READY   STATUS             RESTARTS       AGE
kxta-backend-5797589756-td5b2    1/1     Running            0              5m
kxta-frontend-74ff7cc5c8-wf9jx   1/1     Running            0              5m
kxta-nim-llm-0                        1/1     Running            0              5m
kxta-phoenix-78fd7584b7-s9bwc         1/1     Running            0              5m
```
...
### Required Secrets for KDB-X

| Secret | Purpose | Required |
|--------|---------|----------|
| `ngc-api` | NVIDIA API key for LLMs | Yes |
| `tavily-secret` | Web search fallback | Optional |
| `kdb-license-secret` | PyKX license (internal KDB-X only) | For internal deployment |

### Verify KDB-X Integration

```bash
# Check KDB is enabled
...
1. Delete the KXTA deployment:
```bash
helm delete kxta -n kxta
```
