...
2. Attempt to list the collections through the backend service:
    ...
    ```bash
    # replace kxta-backend with the *PUBLIC* IP address of the backend service, `localhost`, or run this command from a container 
    curl -v http://kxta-backend:3838/v1/collections
    ```

    If this doesn't work, check the backend configuration and ensure `RAG_INGEST_URL` is properly set. Check the backend logs, eg `docker logs kxta-backend -f`. 


3. Attempt to list the collections through the application. Check the browser network logs and the application logs, `docker logs kxta-frontend -f`. If this does not work, ensure you have configured the application via the `INFERENCE_ORIGIN` environment variable which should be set to the IP address of your backend service, `http://kxta-backend:3838`. 
...
For additional known issues related to the RAG Blueprint, see the [CHANGELOG](https://github.com/KxSystems/nvidia-kx-samples/blob/main/KX-nvidia-rag-blueprint/CHANGELOG.md#all-known-issues).
...
Ensure you have provided a valid Tavily API key (`TAVILY_API_KEY`) and/or a Firecrawl key (`FIRECRAWL_API_KEY`). The `web_search` agent prefers Firecrawl and falls back to Tavily; the orchestrator only falls back to web search when no enabled on-premise source returns a relevant answer.

## Source Agents Not Available or Not Selected

The UI agent picker reflects each agent's availability state (`available` / `needs_key` / `unavailable`). Query the registry directly to see why an agent is gated off:

```bash
curl -s http://localhost:3838/source_agents | jq '.[] | {name, status, reason}'
```

Common causes by agent:

| Agent | Appears unavailable when… |
|-------|---------------------------|
| `kdb`, `kdb_pit` | `KDB_ENABLED` unset/false, MCP endpoint unreachable, or (for `kdb_pit`) no `trade`/`quote` tick data loaded — load it via **KDB-X → Load Data** |
| `kdb_docs` | no document collection selected (`PUT /settings/kdb-docs`), KDB-X host unset, or no embedding endpoint configured/keyed (`EMBEDDING_NIM_URL` or hosted + `NVIDIA_API_KEY`) |
| `onetick` | `ONETICK_CLIENT_ID` / `ONETICK_CLIENT_SECRET` not set |
| `market_data`, `news_headlines` | `ALPHAVANTAGE_API_KEY` not set |
| `macro_economic` | `FRED_API_KEY` not set |
| `sec_filings` | reachable by default; set `SEC_EDGAR_EMAIL` to identify your requests |
| `web_search` | neither `FIRECRAWL_API_KEY` nor `TAVILY_API_KEY` set |

**An agent is enabled but never used.** Routing is per-query: the planner tags each query with a preferred source, and `select_sources()` falls back to keyword then semantic similarity. If an agent should have answered but didn't, check the `search_progress` SSE events / backend logs for the chosen sources and any "rerouting to …" messages.

**`kdb_docs` returns nothing / dimension errors.** The selected collection must be ingested at the same embedding dimension the agent queries with (2048-dim for `nvidia/llama-nemotron-embed-1b-v2`). Re-ingest the documents through the RAG ingestor against a 2048-dim collection. See [docs/kdb-docs-agent.md](kdb-docs-agent.md).

**Relevancy gate too strict/lenient.** When `RERANKER_NIM_URL` is set, relevancy is judged by the reranker NIM against `RERANKER_RELEVANCY_THRESHOLD` (logit, default `0.0`); otherwise an LLM judge is used. If good results are being dropped, lower the threshold; if the reranker is unreachable the system logs a fallback to the LLM judge.
...
kubectl -n kxta exec deployment/kxta-backend -- env | grep KDB_ENABLED
...
For additional KDB-X troubleshooting, see the [KDB-X Deployment Guide](./kxta-deployment-guide.md#troubleshooting).
