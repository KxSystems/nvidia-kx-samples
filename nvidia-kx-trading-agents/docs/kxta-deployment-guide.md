...
The blueprint routes each query through a **source-agent registry** of 11 agents
(plus a synthetic web fallback). KDB-X alone backs three of them — one engine for
tick data *and* document vectors:

- **`kdb`** — time-series SQL (stocks, trades, quotes) via the KDB-X MCP server
- **`kdb_docs`** — GPU vector search over documents/filings ingested into KDB-X
- **`kdb_pit`** — point-in-time as-of joins (trade ↔ prevailing quote)

The remaining agents are **`rag`** (RAG document retrieval), **`web_search`**
(Firecrawl/Tavily), **`onetick`**, **`market_data`**, **`news_headlines`**,
**`fundamentals`**, **`sec_filings`**, and **`macro_economic`**. Each is enabled
when its dependency (endpoint, API key, data, or selected collection) is present;
query `GET /source_agents` to see live availability. See the
[Configuration Reference](configuration-reference.md) for the credential each
agent needs.
...
+------------------+              +----------+----------+              +------------------+
|                  |              |                     |              |                  |
|  KDB-X MCP       +<-------------+   KXTA Backend     +------------->+   RAG Server     |
...
- **Tavily / Firecrawl API Key** - For the `web_search` agent (web fallback)
- **KDB-X MCP Endpoint** - For financial data queries (default: KX public endpoint)
- **Source-agent credentials** - `ALPHAVANTAGE_API_KEY` (market data + news), `FRED_API_KEY` (macro), `SEC_EDGAR_EMAIL` (filings identity), `ONETICK_CLIENT_ID`/`ONETICK_CLIENT_SECRET` (OneTick); see the [Configuration Reference](configuration-reference.md)
- **NIM enhancements** - `RERANKER_NIM_URL` (relevancy gate), `EMBEDDING_NIM_URL` (semantic routing + `kdb_docs` vectors), summarization, NemoGuard — each enable-when-present and reusable from a co-located RAG cluster
...
This blueprint ships as **source** (no pre-built images are published from this repo). Build the
backend and frontend images from the included Dockerfiles and push them to your container registry:

```bash
# Build the KDB-enabled images
...
Then set `image.repository` / `frontend.image.repository` (Helm) or `KXTA_BACKEND_IMAGE` /
`KXTA_FRONTEND_IMAGE` (Docker Compose) to the images you pushed.

> **Note:** The base NVCR images (`nvcr.io/nvidia/blueprint/aira-*`) do **not** include the KDB-X
> integration / source agents — build from this repo to get them.
...
#### Option A: Build & push to your registry (recommended)

Build the images (see [Build the Images](#build-the-images) above), push them to your registry, then point the chart at them:
...
> **Note:** The base NVCR images (`nvcr.io/nvidia/blueprint/aira-*`) are **stock AI-Q without the KDB-X integration / source agents** — they are not a substitute for building from this repo.

#### Option B: Docker Hub

Best for quick deployments using a Docker Hub repository.

```yaml
# values-docker-hub.yaml
image:
  repository: docker.io/youruser/kxta-backend
  tag: "v1.2.0"
...
image:
  repository: registry.company.com/aiq/kxta-backend
  tag: "v1.2.0"
...
```bash
helm upgrade --install kxta deploy/helm/kxta \
  -n kxta -f deploy/helm/kxta/examples/values-private-registry.yaml \
...
helm upgrade --install kxta deploy/helm/kxta \
  -n kxta -f deploy/helm/kxta/aiq-values.yaml \
...
helm upgrade --install kxta deploy/helm/kxta \
  -n kxta \
  --set image.repository="us-central1-docker.pkg.dev/project/repo/kxta-backend" \
  --set ngcApiSecret.password="$NVIDIA_API_KEY"
```
...
```bash
kubectl -n kxta create secret docker-registry my-registry-secret \
...
- Two Helm charts: `kxta` + `kdb-x-mcp-server`
...
| Helm Charts | 1 (kxta) | 2 (kxta + kdb-x-mcp-server) |
...
mkdir -p ~/.config/kxta
chmod 700 ~/.config/kxta

# Create environment file
cat > ~/.config/kxta/env << 'EOF'
# NVIDIA API Key - for hosted LLM services
...
# Expected output:
# NAMES                 STATUS
# kxta-frontend       Up X minutes
# kxta-backend        Up X minutes
# kdb-mcp-server        Up X minutes (healthy)
# kdbx                  Up X minutes
# kxta-redis          Up X minutes
...
kubectl -n kxta rollout status deployment/kxta-backend
...
kubectl -n kxta rollout status deployment/kdb-mcp-kdb-x-mcp-server-kdbx --timeout=300s
kubectl -n kxta rollout status deployment/kdb-mcp-kdb-x-mcp-server --timeout=120s
```
...
```bash
# Check pods
kubectl -n kxta get pods -l app.kubernetes.io/instance=kdb-mcp
...
```yaml
# In deploy/helm/kxta/aiq-values.yaml
backendEnvVars:
  # ... other settings ...
...
# Wait for deployment
kubectl -n kxta rollout status deployment/kxta-backend
kubectl -n kxta rollout status deployment/kxta-frontend
```
...
```bash
# Check all pods
kubectl -n kxta get pods

# Expected output:
# NAME                                             READY   STATUS    AGE
# kxta-backend-xxx                          1/1     Running   1m
# kxta-frontend-xxx                         1/1     Running   1m
# kxta-redis-xxx                                 1/1     Running   1m
# kdb-mcp-kdb-x-mcp-server-xxx                     1/1     Running   5m
# kdb-mcp-kdb-x-mcp-server-kdbx-xxx                1/1     Running   5m

# Check services
kubectl -n kxta get svc

# Test backend health
kubectl -n kxta exec deployment/kxta-backend -- curl -s http://localhost:3838/aiqhealth
...
kubectl -n kxta port-forward svc/kxta-frontend 3000:3000 &
kubectl -n kxta port-forward svc/kxta-backend 3838:3838 &
...
```bash
# Check pod logs
kubectl -n kxta logs -f deployment/kxta-backend
...
```bash
# Check MCP server logs
kubectl -n kxta logs deployment/kdb-mcp-kdb-x-mcp-server --tail=50
...
kubectl -n kxta get secret kdb-license-secret -o yaml

# 3. Cannot connect to KDB-X database
#    - Ensure kdb-mcp-kdb-x-mcp-server-kdbx pod is running first
kubectl -n kxta get pods -l app.kubernetes.io/component=kdbx-database
```
...
kubectl -n kxta logs deployment/kdb-mcp-kdb-x-mcp-server-kdbx -c install-kdbx --tail=50
...
kubectl -n kxta logs deployment/kxta-backend | grep -i kdb

# Verify KDB_ENABLED is true
kubectl -n kxta get deployment kxta-backend -o jsonpath='{.spec.template.spec.containers[0].env}' | jq '.[] | select(.name=="KDB_ENABLED")'
...
```bash
# Check frontend logs
kubectl -n kxta logs deployment/kxta-frontend --tail=20
...
kubectl -n kxta get secret

# Verify secret contents
kubectl -n kxta get secret ecr-secret -o jsonpath='{.data.\.dockerconfigjson}' | base64 -d
...
kubectl -n kxta delete secret ecr-secret 2>/dev/null || true
kubectl -n kxta create secret docker-registry ecr-secret \
...
kubectl -n kxta delete secret <secret-name>
...
```bash
# View all resources
kubectl -n kxta get all

# Describe pod for events
kubectl -n kxta describe pod <pod-name>

# Stream backend logs
kubectl -n kxta logs -f deployment/kxta-backend

# Interactive shell in backend
kubectl -n kxta exec -it deployment/kxta-backend -- /bin/bash

# Check Redis connectivity
kubectl -n kxta exec -it deployment/kxta-backend -- redis-cli -h kxta-redis ping

# Check KDB-X database is running with sample data
kubectl -n kxta logs deployment/kdb-mcp-kdb-x-mcp-server-kdbx -c kdbx --tail=20
# Should show: "KDB-X initialized with sample data (2023-2024)"

# Force pod recreation after config changes
kubectl -n kxta delete pods -l app.kubernetes.io/instance=kdb-mcp
kubectl -n kxta delete pods -l app.kubernetes.io/instance=kxta
```
...
helm history kxta -n kxta
helm history kdb-mcp -n kxta

# Rollback to previous version
helm rollback kxta -n kxta
helm rollback kdb-mcp -n kxta

# Complete cleanup and fresh install
helm uninstall kxta -n kxta
helm uninstall kdb-mcp -n kxta
kubectl -n kxta delete secrets --all
kubectl -n kxta delete pvc --all
```
...
```bash
kubectl -n kxta port-forward svc/kxta-phoenix 6006:6006
...
```bash
# Install
helm upgrade --install kxta deploy/helm/kxta -n kxta -f values.yaml

# Upgrade
helm upgrade kxta deploy/helm/kxta -n kxta -f values.yaml
...
```bash
# Scale deployment
kubectl -n kxta scale deployment/kxta-backend --replicas=3

# Restart deployment
kubectl -n kxta rollout restart deployment/kxta-backend

# View logs with timestamps
kubectl -n kxta logs -f deployment/kxta-backend --timestamps
...
kubectl -n kxta create secret generic ngc-api \
...
kubectl -n kxta get secrets

# Check secret keys (without exposing values)
kubectl -n kxta get secret ngc-api -o jsonpath='{.data}' | jq -r 'keys[]'

# Verify secret is not empty (shows first 10 chars)
kubectl -n kxta get secret ngc-api -o jsonpath='{.data.NVIDIA_API_KEY}' | base64 -d | head -c 10; echo "..."

# Check if secret is mounted in pod
kubectl -n kxta exec deployment/kxta-backend -- env | grep -E "INSTRUCT_API_KEY|NEMOTRON_API_KEY"
```
...
kubectl -n kxta get secret ngc-api -o jsonpath='{.data.NVIDIA_API_KEY}' | base64 -d | wc -c
# Should return > 0
