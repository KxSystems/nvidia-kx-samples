...
> reflection used in production, not by RAG alone. Configure which sources are
> enabled the same way you would for a live run.
...
- **Helm**: Deploy via Helm using [get-started-helm.md](get-started/get-started-helm.md) for Kubernetes environments
...
export INSTRUCT_BASE_URL="http://localhost:8050/v1"

# Hosted Nemotron Backend (for reasoning)
export NEMOTRON_BASE_URL="http://localhost:8999/v1"
...
# Full workflow + evaluation (requires RAG server) + kxta-instruct-llm
...
**Reference Examples**: Look at the existing evaluators in `kxta/src/kxta/eval/evaluators/` for complete implementation patterns:
...
Create a new Python file for your evaluator (e.g., `my_custom_evaluator.py`) inside `kxta/src/kxta/eval/evaluators/`. Use the `BaseEvaluator` class for simplified implementation:

```python
# kxta/src/kxta/eval/evaluators/my_custom_evaluator.py
import asyncio
import logging
...
class MyCustomEvaluator(BaseEvaluator):
    """Custom evaluator following the KXTA evaluation pattern."""
...
Register your evaluator using the function-based pattern. In `kxta/src/kxta/eval/evaluator_register.py`, add the following:

```python
# kxta/src/kxta/eval/evaluator_register.py

# ... existing imports
from nat.builder.framework_enum import LLMFrameworkEnum
from kxta.eval.evaluators.my_custom_evaluator import MyCustomEvaluator, MyCustomEvaluatorConfig
...
- Follow the same pattern as existing KXTA evaluators

### 4. Import for Registration

To ensure the evaluator is registered at runtime, import the evaluator function in `kxta/src/kxta/eval/__init__.py` — even if the function is not called directly:

```python
# kxta/src/kxta/eval/__init__.py
...
To ensure all your changes are picked up, reinstall the `kxta` package in editable mode:
...
- **Follow existing patterns**: Reference the evaluators in `kxta/src/kxta/eval/evaluators/` for complete examples
...
After running your evaluation, use the [AI-Q Evaluation Tutorial Notebook](../notebooks/kxta_evaluation_tutorial.ipynb) to:
