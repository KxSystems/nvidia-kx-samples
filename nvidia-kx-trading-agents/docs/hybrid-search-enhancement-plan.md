...
> has since moved to a **source-agent registry** with per-query routing, which
> supersedes the fixed KDB+RAG branch and the merge-strategy/weight knobs below.
> The roadmap items have largely shipped in a different form. Use
> [docs/sequence-diagram.md](sequence-diagram.md),
> [docs/whats-different-from-aiq.md](whats-different-from-aiq.md), and the
> [README](../README.md) for the current architecture. Kept for historical context.
>
> **Roadmap → reality:**
>
> | Roadmap item (below) | Shipped as |
> |---|---|
> | Phase 1 — Semantic query classification | `select_sources()` planner-tag → keyword → **semantic similarity (embedding NIM)** routing in `search_utils.py` / `source_agents/routing.py` |
> | Phase 3 — KDB-X vector search | the **`kdb_docs`** agent (GPU cuVS/CAGRA vector search in KDB-X) |
> | Phase 4 — Result synthesis / relevancy | **reranker NIM** relevancy gate + failure-aware **reroute** + supervisor reflection digest |
> | Phase 1 — confidence-based source inclusion | per-query routing with bounded (≤2) fallback sources |
> | "Hybrid mode" / merge weights | replaced by `merge_source_results()` over the chosen agents; the `HYBRID_*` env vars below were never implemented |
>
> The KDB+/RAG-only framing, the `hybrid_mode`/`hybrid_config` request fields, and
> the `HYBRID_SEARCH_*` / `*_SEARCH_WEIGHT` environment variables in this document
> are **not** part of the current system.

## Original Implementation (v1.0 — superseded)
...
- `kxta/src/kxta/search_utils.py` - Core hybrid search logic
- `kxta/src/kxta/prompts.py` - Query planner instructions for hybrid mode
